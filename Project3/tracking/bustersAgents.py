    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # firstly find the closest ghost
        distances = []
        positions = []
        ...
        ghostPos = positions[index]

        newDistances = []
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            newDistance = self.distancer.getDistance(successor, ghostPos)
            newDistances.append(newDistance)
        index = newDistances.index(min(newDistances))
        bestAction = legal[index]
        return bestAction
